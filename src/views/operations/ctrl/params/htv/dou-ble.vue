...
const oneCompiled_url = 'api/gOps/setInactive/'
...
const changeOneCompiled = ()=>{
    ...
    oneCompiledData.is_inactive = toValue(isDecimal)
    getOneCompiled()
}
watch(oneCompiled, (value)=>{
    ...
    oneCompiledData.is_inactive = value?.is_inactive;

    codeMed.value = value?.code_med ?? '';
    isDecimal.value = value?.is_inactive ?? false;
})
</script>
